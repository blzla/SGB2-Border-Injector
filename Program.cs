...
namespace SGB2_Border_Injector
{
    class Program
    {
        private static readonly (string name, int tilemap, (int addr, int chunk_length)[] tileset_chunks, int tileset_maxsize, int palettes, (int addr, byte value)[] patches, int icon, int checksum)[] border_data = new (string, int, (int, int)[], int, int, (int, byte)[], int, int)[] {
            ...
            ("Windows (SGB1)", 0x3DA00, new (int, int)[] { (0x18000, 0x3474) }, 0x3308, 0x1B4F4, new (int, byte)[] { (0x9C1F, 0x07), (0x9C25, 0x00), (0x9C26, 0xDA), (0x9CF3, 0x60), (0x9C5C, 0x60) }, 0x12690, -1),
            ("Cork Board (SGB1)", 0x27A20, new (int, int)[] { (0x23B20, 0x257B) }, 0x257B, 0x2611C, new (int, byte)[] { (0xB7EA, 0x20), (0xB7EB, 0xFA), (0xB7BA, 0x20), (0xB7BB, 0xBB), (0xB83E, 0x60) }, 0x126D0, -1),
            ("Log Cabin In The Countryside (SGB1)", 0x1C660, new (int, int)[] { (0x79000, 0x3300) }, 0x3300, 0x1D5E8, new (int, byte)[] { (0xA6B7, 0x0F), (0xA6BD, 0x00), (0xA6BE, 0x90), (0xA773, 0x80), (0xA774, 0x71), (0xA823, 0x80), (0xA824, 0x0D) }, 0x12710, -1),
            ("Movie Theater (SGB1)", 0x2C6C8, new (int, int)[] { (0x7C300, 0x3300) }, 0x3300, 0x2D8BC, new (int, byte)[] { (0xCF7B, 0xC8), (0xCF7C, 0xC6), (0xCF45, 0x0F), (0xCF4B, 0x00), (0xCF4C, 0xC3), (0xCFFB, 0x60), (0xCFE2, 0x60), (0xCF0C, 0x60), (0xCA4B, 0x60) }, 0x12750, -1),
            ("Cats (SGB1)", 0x1F716, new (int, int)[] { (0x20F73, 0x2BAC) }, 0x2BAC, 0x20DF3, new (int, byte)[] { (0xB90D, 0x03), (0xB917, 0x16), (0xB918, 0xF7), (0xB8DD, 0x04), (0xB8E3, 0x73), (0xB8E4, 0x8F), (0xB945, 0x80), (0xB946, 0x5B), (0xB9B5, 0x60) }, 0x12790, -1),
            ("Chequered Desk With Pencils (SGB1)", 0x58B80, new (int, int)[] { (0x45340, 0x2CC0) }, 0x2CC0, 0x29C21, new (int, byte)[] { (0xD3CE, 0x0B), (0xD3D8, 0x80), (0xD3D9, 0x8B), (0xD37B, 0x08), (0xD381, 0x40), (0xD382, 0xD3), (0xD406, 0x80), (0xD407, 0x5B) }, 0x127D0, -1),
            ("Escher (SGB1)", 0x20000, new (int, int)[] { (0x159C0, 0x2640) }, 0x2640, 0x1E78A, new (int, byte)[] { (0xC734, 0x04), (0xC73E, 0x00), (0xC73F, 0x80), (0xC704, 0x02), (0xC70A, 0xC0), (0xC70B, 0xD9), (0xC81A, 0x60), (0xC794, 0x60) }, 0x123D0, -1)
        };
        ...
        internal static bool InjectCustomBorder(string sgb2_rom, string border_file, int border, int icon, bool set_startup, bool dither, bool external_palettes, bool backup)
        {
            ...
            File.WriteAllBytes("tilemap_decompressed.bin", SGBCompression.Decompress(tilemap));
            File.WriteAllBytes("tileset_compressed.bin", SGBCompression.Compress(tileset));
            File.WriteAllBytes("tileset_decompressed.bin", SGBCompression.Decompress(tileset));
                */
#endif
        ...
        private static bool CreateOldFullDMARoutine(FileStream fs)
        {
            try
            {
                fs.Seek(0x85FD, SeekOrigin.Begin);
        ...
        private static bool PatchDMARoutines(FileStream fs, int border)
        {
            try
            {
                ...
                else
                {
                    ...
                    fs.Write(new byte[] { 0x20, 0xFD, 0x85 }, 0, 0x03);
                    fs.WriteByte(0x82);
        ...
        private static bool DisableScreensaverSlot(FileStream fs, int border)
        {
            try
            {
                ...
                // asm changes
                // re-enable screensavers if they were disabled in a previous version
